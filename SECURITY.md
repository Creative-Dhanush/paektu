...
the subject line `plumbline security`. Please do not open a public issue for a
...
Plumbline is pre-1.0 and only the latest release receives fixes.
...
Plumbline reads files from a repository path you point it at, parses YAML, and
...
- Evidence artifacts that can be altered without `plumbline verify` noticing
...
- Someone editing `plumbline.yaml` to make a declared control pass. That is the
  documented trust model, not a flaw. See `docs/guides/documentation-drift.md`.

## What this tool does not do

Plumbline does not make you compliant, and a green run is not an audit opinion.
